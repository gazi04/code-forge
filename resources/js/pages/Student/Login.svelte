...
<div
    class="login-container bg-[#0d071d] text-white p-8 rounded-2xl max-w-md mx-auto border border-indigo-900/50 shadow-2xl mt-20 font-sans"
>
    <div class="text-center mb-8">
        <h1 class="text-3xl font-black mb-2 tracking-wide text-indigo-100">
            Student Portal
        </h1>
        <p class="text-indigo-300/60 text-sm">
            Enter your credentials to access the databanks.
        </p>
    </div>
    ...
    <form onsubmit={submitLogin} class="space-y-4">
        <div>
            <label
                for="email"
                class="block text-xs font-mono uppercase tracking-widest text-indigo-400 mb-1"
                >Email Address</label
            >
            <input
                id="email"
            ...
            {#if form.errors.email}
                <p class="text-rose-400 text-[11px] mt-1 font-mono font-bold">
                    {form.errors.email}
                </p>
            {/if}
        </div>
