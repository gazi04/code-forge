...
<Layout theme={themeData}>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
    <header class="mb-12 border-b border-[color-mix(in_srgb,var(--text-color)_10%,transparent)] pb-12">
    ...
    <div class="relative z-10">
      <h2 class="text-sm font-mono text-[var(--text-color)] opacity-40 uppercase tracking-widest mb-8">Available Curriculum</h2>

      {#if courses.length === 0}
        <div class="p-12 bg-surface text-center shadow-inner rounded-xl">
          <p class="text-[var(--text-color)] opacity-30 font-mono text-sm">System empty. Awaiting Dungeon Master configurations...</p>
        </div>
      {:else}
        <svelte:component this={layoutRegistry[layoutPreference]} {courses} />
      {/if}
    </div>
  </div>
</Layout>
