...
<div
    class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 pb-24"
>
    {#each courses as course, i (course.slug)}
        {#if userLevel >= (course.min_level_requirement || 1)}
            ...
            >
                ...
                <div class="relative z-10 flex-1">
                    <h3
                        class="text-xl font-bold text-[var(--text-color)] mb-3 group-hover:text-[var(--primary-color)] transition-colors duration-300"
                    >
                        {course.name}
                    </h3>
                    <p
                        class="text-sm text-[var(--text-color)] opacity-60 line-clamp-3"
                    >
                        {course.description ||
                ...
                <div
                    class="mt-8 pt-4 border-t border-[color-mix(in_srgb,var(--text-color)_10%,transparent)] flex items-center justify-between opacity-60 group-hover:opacity-100 transition-opacity"
                >
                    <span
                        class="text-xs font-mono uppercase tracking-wider text-[var(--primary-color)]"
                        >Initialize →</span
                    >
                </div>
            </Link>
        {:else}
            ...
            >
                ...
                <div
                    class="mt-8 pt-4 border-t border-red-900/30 flex items-center justify-between opacity-80"
                >
                    <span
                        class="text-xs font-mono uppercase tracking-wider text-red-500 font-bold"
                    >
