<script>
  ...
  function scrollToIdx(idx) {
    if (idx < 0 || idx >= courses.length || !container) return;
  ...
  <div class="relative px-4 sm:px-12 md:px-16">
    ...
    >
      {#each courses as course, i}
        ...
        >
          <span class="text-xs font-mono text-[var(--primary-color)] mb-4 block uppercase tracking-widest font-bold">
            Sector {String(i + 1).padStart(2, '0')}
          </span>

          <h3 class="text-2xl font-black text-[var(--text-color)] mb-3 group-hover:text-[var(--primary-color)] tracking-wide transition-colors duration-300">
            {course.name}
          </h3>

          <p class="text-sm text-[var(--text-color)] opacity-60 line-clamp-3 mb-6">
            {course.description || 'Initialize this sector checkpoint matrix to unlock its specific algorithmic challenges.'}
          </p>

          <div class="mt-auto pt-4 border-t border-[color-mix(in_srgb,var(--text-color)_5%,transparent)] flex items-center gap-2 text-[var(--text-color)] opacity-50 group-hover:opacity-100 group-hover:text-[var(--primary-color)] transition-all duration-300">
