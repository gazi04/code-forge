...
<div class="layout-container" style={cssVariables}>
    ...
    <nav class="sticky top-0 z-50 border-b border-white/10 bg-black/60 backdrop-blur-2xl text-white transition-colors duration-500">
        <div class="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2 md:gap-3 text-xs font-bold uppercase tracking-wider hidden sm:flex">
                {#if user}
                    {#if user.streak_count > 0}
                        <div class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-orange-500/10 border border-orange-500/20 text-orange-400 font-mono shadow-[0_0_10px_rgba(249,115,22,0.1)]">
                            <span class="text-sm leading-none">🔥</span> {user.streak_count}
                        </div>
                    {/if}

                    <div class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 font-mono shadow-[0_0_10px_rgba(234,179,8,0.1)]">
                        <span class="text-sm leading-none">💰</span> {user.coins}
                    </div>

                    <div class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[color-mix(in_srgb,var(--primary-color)_10%,transparent)] border border-[color-mix(in_srgb,var(--primary-color)_30%,transparent)] text-[var(--primary-color)] font-mono shadow-[0_0_10px_color-mix(in_srgb,var(--primary-color)_20%,transparent)] transition-colors duration-500">
                        <span class="text-sm leading-none">✨</span> {user.xp} XP
                    </div>

                    <div class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[color-mix(in_srgb,var(--accent-color)_10%,transparent)] border border-[color-mix(in_srgb,var(--accent-color)_40%,transparent)] text-[var(--accent-color)] shadow-[0_0_10px_color-mix(in_srgb,var(--accent-color)_20%,transparent)] transition-colors duration-500">
                        Lv. {user.level}
                    </div>

                    <div class="flex items-center gap-2 pl-3 ml-2 border-l border-white/10">
                        <div class="w-8 h-8 rounded-full bg-gradient-to-tr from-[var(--primary-color)] to-[var(--accent-color)] flex items-center justify-center text-black font-black text-sm shadow-inner transition-colors duration-500">
                            {user.name.charAt(0)}
                        </div>
