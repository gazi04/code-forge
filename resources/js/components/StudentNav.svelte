...
>
    <div
        class="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-2 sm:gap-4"
    >
        <!-- Logo -->
        <Link
        ...
        {#if user}
            ...
            >
                ...
                <span
                    class="hidden md:inline font-mono text-[10px] uppercase tracking-widest flex-1 text-left"
                    >Search</span
                >
                <kbd
                    class="hidden lg:inline text-[9px] px-1.5 py-0.5 rounded border border-[color-mix(in_srgb,var(--text-color)_12%,transparent)] bg-[color-mix(in_srgb,var(--text-color)_4%,transparent)] font-mono shrink-0"
                    >⌘K</kbd
                >
            </button>
            ...
            <div class="flex items-center gap-1.5 sm:gap-2 min-w-0">
                <!-- Stats -->
                <div
                    class="flex items-center gap-1.5 px-2 py-1.5 sm:gap-2 lg:gap-3 lg:px-3 rounded-lg bg-[color-mix(in_srgb,var(--text-color)_4%,transparent)] border border-[color-mix(in_srgb,var(--text-color)_7%,transparent)] font-mono text-[11px] sm:text-xs min-w-0 shrink whitespace-nowrap"
                >
                    <span
                        class="text-[var(--accent-color)] font-black transition-colors duration-500"
                        >Lv. {user.level}</span
                    >
                    <span
                        class="text-[color-mix(in_srgb,var(--text-color)_15%,transparent)]"
                        >·</span
                    >
                    <span class="text-yellow-400 font-bold"
                        >💰 {user.coins.toLocaleString()}</span
                    >
                    {#if user.streak_count > 0}
                        ...
                        {#if user.streak_at_risk}
                            <span class="text-red-400 font-bold animate-pulse"
                                >⚠️ {user.streak_count} 🔥</span
                            >
                        {:else}
                ...
                <!-- Divider -->
                <div
                    class="hidden lg:block w-px h-6 bg-[color-mix(in_srgb,var(--text-color)_10%,transparent)] mx-1"
                ></div>
                ...
                <!-- Divider -->
                <div
                    class="hidden md:block w-px h-6 bg-[color-mix(in_srgb,var(--text-color)_10%,transparent)] mx-1"
                ></div>
                ...
                >
                    ...
                    <div class="hidden md:flex flex-col leading-tight min-w-0">
                        <span
                            class="text-xs font-bold text-[var(--text-color)] truncate max-w-[100px]"
                            >{user.name}</span
                        >
                        {#if user.equipped?.title}
                            <span
                                class="text-[10px] font-mono truncate max-w-[100px]"
                                style="color: {user.equipped.title.color ??
                                    'inherit'}">{user.equipped.title.name}</span
                            >
                        {/if}
                    </div>
                </Link>
        ...
        {:else}
            <div
                class="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 flex items-center gap-2 text-xs font-mono"
            >
