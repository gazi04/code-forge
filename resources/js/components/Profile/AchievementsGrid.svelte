...
<div class="bg-surface rounded-2xl p-6 mb-6">
    <h2 class="text-lg font-black text-white mb-1 tracking-tight">Achievements</h2>
    <p class="text-xs font-mono text-white/40 uppercase tracking-widest mb-5">
        {unlocked.length} / {achievements.length} Unlocked
    </p>

    {#if achievements.length === 0}
        <p class="text-sm text-white/30 font-mono text-center py-8">No achievements configured yet.</p>
    {:else}
        <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
            {#each unlocked as achievement (achievement.id)}
                ...
                >
                    ...
                    <span class="text-[11px] font-bold text-white text-center leading-tight line-clamp-2">
                        {achievement.name}
                    </span>

                    {#if achievement.unlocked_at}
                        <span class="text-[9px] font-mono text-[var(--accent-color)] text-center">
                    ...
                    >
                        <p class="font-bold text-white mb-1">{achievement.name}</p>
                        {#if achievement.description}
                            <p class="text-white/60 leading-snug">{achievement.description}</p>
                        {/if}
                    </div>
                </div>
            {/each}
            ...
            {#each locked as achievement (achievement.id)}
                ...
                >
                    ...
                    <span class="text-[11px] font-bold text-white/25 text-center leading-tight line-clamp-2">
                        {achievement.name}
                    </span>
                    ...
                    >
                        <p class="font-bold text-white mb-1">{achievement.name}</p>
                        {#if achievement.description}
                            <p class="text-white/60 leading-snug mb-2">{achievement.description}</p>
                        {/if}
                        <p class="text-white/30 italic text-[10px]">🔒 Not yet unlocked</p>
                    </div>
                </div>
            {/each}
        </div>
...
<style>
    ...
    .achievement-card.unlocked:hover {
        ...
        box-shadow: 0 0 16px rgba(var(--primary-color), 0.3), var(--card-shadow);
    }
    .achievement-tooltip {
