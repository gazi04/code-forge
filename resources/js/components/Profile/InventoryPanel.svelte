<script>
    ...
    let byType = $derived(
        GROUPS.reduce((acc, group) => {
            acc[group.key] = inventory.filter(
                (inv) => inv.store_item.type === group.key,
            );
    ...
    function handleActivate(inventoryId) {
        router.post(
            `/inventory/${inventoryId}/activate`,
            {},
            { preserveScroll: true },
        );
    }
...
<div class="bg-surface rounded-2xl p-6 mb-6">
    <h2
        class="text-sm font-black uppercase tracking-widest text-[color-mix(in_srgb,var(--text-color)_40%,transparent)] mb-5"
    >
        Inventory
    </h2>
    ...
    {#if !hasItems}
        <div
            class="text-center py-10 text-[color-mix(in_srgb,var(--text-color)_30%,transparent)] font-mono text-sm"
        >
        ...
        <div class="space-y-6">
            {#each GROUPS as group (group.key)}
                ...
                {#if groupItems.length > 0}
                    <section>
                        <h3
                            class="text-xs font-black uppercase tracking-widest text-[color-mix(in_srgb,var(--text-color)_35%,transparent)] mb-3"
                        >
                            {group.label}
                        </h3>
                        <div
                        ...
                        >
                            {#each groupItems as inv (inv.id)}
                                <ItemCard
                                    ...
                                    mode="inventory"
                                    owned={true}
                                    equipped={equipped[inv.store_item.type] ===
                                        inv.store_item_id}
                                    affordable={true}
                                    ...
                                    onUnequip={() =>
                                        handleUnequip(inv.store_item.type)}
