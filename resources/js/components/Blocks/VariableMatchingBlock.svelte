<script>
  ...
  let networkFeedback = $state("Select a node from the left matrix, then find its matching anchor on the right.");
  ...
  function initializeMatrix() {
    ...
    let leftSide = data.pairs.map((pair, idx) => ({ id: idx, text: pair.left_item }));
    let rightSide = data.pairs.map((pair, idx) => ({ id: idx, text: pair.right_item }));
    ...
    movesCount = 0;
    isCleared = false;
    networkFeedback = "Select a node from the left matrix, then find its matching anchor on the right.";
    feedbackStatus = "info";
  }

  function handleLeftClick(id) {
    if (isCleared || matchedIds.includes(id)) return;
    ...
    if (selectedLeftId === id) {
      ...
      selectedLeftId = id;
      networkFeedback = "Left terminal locked. Complete the bridge circuit on the right column.";
      feedbackStatus = "info";
    }
  }

  function handleRightClick(id) {
    if (isCleared || matchedIds.includes(id) || selectedLeftId === null) return;
    ...
    if (selectedLeftId === id) {
      ...
      selectedLeftId = null;
      networkFeedback = "💥 Link Refused! Relational constraints do not match.";
  ...
  <div class="p-6 w-full">
    ...
    <div class="flex justify-between items-center mb-6 text-xs font-mono">
      <div class="px-3 py-1.5 bg-[#150b2e] border border-indigo-900/50 text-indigo-300 rounded-md">
        Bonds Connected: <span class="font-bold text-white">{matchedIds.length} / {totalPairsCount}</span>
      </div>

      <button onclick={initializeMatrix} disabled={isCleared}
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 relative w-full bg-[#0a0515] p-6 rounded-xl border border-indigo-950/80 shadow-inner">

      <div class="hidden md:block absolute top-6 bottom-6 left-1/2 -translate-x-1/2 w-px bg-indigo-950/40 border-dashed border-l border-indigo-900/20"></div>

      <div class="flex flex-col gap-3">
        <span class="text-[10px] text-indigo-500 font-mono uppercase tracking-widest font-bold block mb-1 px-1">Source Relic Matrices</span>
        {#each leftNodes as leftItem (leftItem.id)}
          ...
          >
            <div class="flex justify-between items-center">
              <span class="tracking-wide font-bold">{leftItem.text}</span>
              {#if isAlreadyMatched}
                <span class="text-[10px] text-emerald-500/70 font-bold">🔒 Bound</span>
              {:else if isSelected}
      ...
      <div class="flex flex-col gap-3">
        <span class="text-[10px] text-indigo-500 font-mono uppercase tracking-widest font-bold block mb-1 px-1">Anchor Alignment Deck</span>
        {#each rightNodes as rightItem (rightItem.id)}
          {@const isAlreadyMatched = matchedIds.includes(rightItem.id)}
          {@const isInteractable = selectedLeftId !== null && !isAlreadyMatched}
          ...
          >
            <div class="flex justify-between items-center">
              ...
              {#if isAlreadyMatched}
                <span class="text-xs text-emerald-400/80">✓ Connected</span>
              {/if}
            </div>
          </div>
        {/each}
    ...
    <div class="w-full mt-4 p-3 rounded-lg text-xs font-mono text-center border transition-all duration-300
      ...
      {feedbackStatus === 'error' ? 'bg-rose-950/40 text-rose-400 border-rose-800/50 animate-shake' : ''}
