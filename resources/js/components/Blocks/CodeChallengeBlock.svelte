...
<div class="bg-surface rounded-2xl overflow-hidden border border-[color-mix(in_srgb,var(--text-color)_10%,transparent)] shadow-xl mt-8">

  <div class="bg-[color-mix(in_srgb,var(--bg-color)_50%,transparent)] px-4 py-3 border-b border-[color-mix(in_srgb,var(--text-color)_5%,transparent)] flex justify-between items-center">
    <div class="flex items-center gap-2">
      <span class="text-[var(--primary-color)]">⚡</span>
      <span class="font-mono text-sm font-bold text-[var(--text-color)] opacity-80 uppercase tracking-widest">
        Arcane Terminal ({data.language})
      </span>
    </div>

    {#if !pyodideReady}
      <span class="text-xs font-mono text-amber-400 animate-pulse">Summoning Interpreter...</span>
    {/if}
  </div>

  <div class="grid grid-cols-1 lg:grid-cols-2">
    <div class="relative border-r border-[color-mix(in_srgb,var(--text-color)_5%,transparent)]">
      <textarea
        bind:value={userCode}
    ...
    <div class="flex flex-col h-[400px]">
      ...
      <div class="flex-1 p-6 bg-[#0a0a0a] overflow-y-auto">
        <div class="font-mono text-xs text-zinc-500 uppercase tracking-widest mb-2">Standard Output</div>
        <pre class="font-mono text-sm text-green-400 whitespace-pre-wrap">{terminalOutput}</pre>
      </div>

      {#if testResults.length > 0}
        <div class="h-1/2 p-6 bg-[color-mix(in_srgb,var(--bg-color)_30%,transparent)] border-t border-[color-mix(in_srgb,var(--text-color)_5%,transparent)] overflow-y-auto">
          <div class="font-mono text-xs text-zinc-500 uppercase tracking-widest mb-4">Quest Objectives</div>
            ...
            {#each testResults as test}
              {#if !test.is_hidden}
                <div class="flex items-center gap-3 text-sm">
                  ...
                  {:else if test.passed}
                    <div class="w-5 h-5 rounded-full bg-[var(--accent-color)] flex items-center justify-center text-[var(--bg-color)] text-xs">✓</div>
                  {:else}
                    <div class="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs">✕</div>
                  {/if}
                  <span class="font-mono opacity-80" class:opacity-40={test.passed === null}>{test.name}</span>
                </div>
              {/if}
            {/each}
          </div>
