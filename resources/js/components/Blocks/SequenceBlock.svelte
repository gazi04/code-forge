<script>
  ...
  let gameFeedback = $state("Click an item to select it, then click another to swap their positions.");
  ...
  onMount(() => {
    ...
    while (shuffled.join('|||') === correctOrder.join('|||') && shuffled.length > 1) {
  ...
  function evaluateSequence() {
    ...
    if (isCorrect) {
      ...
      gameFeedback = "Sequence adjusted. The sequence matrix is still unstable—keep sorting!";
      feedbackStatus = "info";
    }
  }
  ...
  function resetSequence() {
    ...
    attemptsCount = 0;
    isCleared = false;
    gameFeedback = "Click an item to select it, then click another to swap their positions.";
    feedbackStatus = "info";
  }
</script>
  ...
  <div class="p-6 w-full">
    ...
    <div class="flex justify-between items-center mb-4 text-xs font-mono">
      <div class="px-3 py-1.5 bg-[#150b2e] border border-indigo-900/50 text-indigo-300 rounded-md">
        Swaps Made: <span class="font-bold text-white">{attemptsCount}</span>
      </div>

      <button onclick={resetSequence} disabled={isCleared}
    ...
    <div class="w-full bg-[#0a0515] rounded-xl p-4 border border-indigo-950/80 shadow-inner flex flex-col gap-2">
      ...
      {#each dynamicList as elementValue, idx (elementValue + '-' + idx)}
        ...
        >
          <div class="w-6 h-6 rounded-md bg-black/40 border border-indigo-900/40 flex items-center justify-center text-[11px] font-bold text-indigo-400/70">
            {idx + 1}
          </div>
          ...
          <div class="w-2 h-2 rounded-full transition-all duration-300
            {isCleared ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : isCurrentSelection ? 'bg-purple-500 shadow-[0_0_8px_#a855f7]' : 'bg-indigo-950'}">
          </div>
        </div>
      {/each}
    ...
    <div class="w-full mt-4 p-3 rounded-lg text-xs font-mono text-center border transition-all
      {feedbackStatus === 'success' ? 'bg-emerald-950/40 text-emerald-400 border-emerald-800/50' : 'bg-indigo-950/40 text-indigo-300 border-indigo-900/50'}">
      {gameFeedback}
    </div>
